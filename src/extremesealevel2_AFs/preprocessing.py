...
def resample_data(rawdata, resample_freq):
    ...
    h_means = (
        rawdata.resample("h").mean()
    )  # as most records provide hourly obs, this is often the mean of 1 obs = the obs
    h_means.loc[h_means["use_flag"] != 1, "sea_level"] = (
        np.nan
    )  # set hours during which 1 or more observations are bad to nan
    ...
    if resample_freq == "H_mean":
        ...
        d_means = d_means[
            hours_per_day["sea_level"] >= 12
        ]  # only use days with 12 hourly means or more
...
def extract_GESLA2_locations(path_to_gesla2, min_yrs):
    ...
    for this_file in geslafiles:
        ...
        this_name, this_lat, this_lon, start_date, end_date = readmeta_GESLA2(
            os.path.join(gesladir, this_file)
        )

        if (
            (pd.to_datetime(end_date).year - pd.to_datetime(start_date).year)
            < min_yrs - 1
        ) or (pd.to_datetime(end_date).year < 2000):
            continue
...
def extract_GESLA3_locations(path_to_gesla3, min_yrs):
    ...
    meta = meta[
        (meta["GAUGE TYPE"] == "Coastal")
        & (meta["OVERALL RECORD QUALITY"] == "No obvious issues")
        & (meta["NUMBER OF YEARS"] >= min_yrs)
        & (pd.to_datetime(meta["END DATE/TIME"], format="mixed").dt.year >= 2000)
    ].reset_index(
        drop=True
    )  # only consider coastal gauges without issues with sufficient length
...
def reference_data_to_msl(period, data, fn):
    ...
    if len(np.unique(data_in_msl_period.index.date)) < 0.5 * 365.25 * (
        int(y1) - int(y0)
    ):
        print(
            'Warning: Could not reference observations to MSL for {0} because observations are avaible on less than half of all days during "msl_period".'.format(
                fn
            )
        )
        vdatum = "original"
...
def ingest_GESLA3_files(gesla3_path, preproc_settings, fns=None):
    ...
    for fn in fns:
        ...
        if preproc_settings["ref_to_msl"]:
            rawdata, vdatum = reference_data_to_msl(
                preproc_settings["msl_period"].split(","), rawdata, fn
            )
        else:
            vdatum = "original"
...
def ingest_GESLA2_files(gesla2_path, preproc_settings, fns=None):
    ...
    for fn in fns:
        ...
        if (metadata[-1] - metadata[-2]) / np.timedelta64(1, "s") / (
            365.25 * 24 * 3600
        ) < min_yrs:
            continue
        ...
        # compute MSL from raw data if referencing to MSL
        if preproc_settings["ref_to_msl"]:
            data, vdatum = reference_data_to_msl(
                preproc_settings["msl_period"].split(","), data, fn
            )
        else:
            vdatum = "original"
...
def detrend_gesla_dfs(dfs):
    ...
    for k, df in detrended_dfs.items():
        ...
        df["sea_level"] = (
            df["sea_level"] - trend + np.mean(trend)
        )  # subtract trend without changing the vertical datum (mean of timeseries)
...
def deseasonalize_gesla_dfs(dfs):
    ...
    for k, df in deseasonalized_dfs.items():
        monthly_means_at_timesteps = (
            df.groupby(df.index.month).transform("mean")["sea_level"].astype("float64")
        )  # contains mean of all timesteps in month for all years together at each timestep in that month
        df["sea_level"] = (
            df["sea_level"]
            - monthly_means_at_timesteps
            + np.mean(monthly_means_at_timesteps)
        )  # subtract without changing the vertical datum (mean of timeseries)
...
def deseasonalize_ds(ds, variable):
    ...
    deseasoned_ds = ds[variable].groupby(ds.time.dt.month) - ds[variable].groupby(
        ds.time.dt.month
    ).mean("time")

    deseasoned_ds = deseasoned_ds + (
        ds[variable].mean(dim="time") - deseasoned_ds.mean(dim="time")
    )
...
def subtract_amean_from_ds(ds, variable):
    ...
    ds_no_amean = ds[variable].groupby(ds.time.dt.year) - ds[variable].groupby(
        ds.time.dt.year
    ).mean("time")

    ds_no_amean = ds_no_amean + (
        ds[variable].mean(dim="time") - ds_no_amean.mean(dim="time")
    )
...
def subtract_amean_from_gesla_dfs(dfs):
    ...
    for k, df in dfs_no_amean.items():
        annual_means_at_timesteps = (
            df.groupby(df.index.year).transform("mean")["sea_level"].astype("float64")
        )
        df["sea_level"] = (
            df["sea_level"]
            - annual_means_at_timesteps
            + np.mean(annual_means_at_timesteps)
        )  # subtract without changing the vertical datum (mean of timeseries)
    return dfs_no_amean
        ...
        distances = (
            ...
            * np.arcsin(
                np.sqrt(
                    ...
                    + np.cos((np.pi / 180) * lat)
                    * np.cos((np.pi / 180) * lats)
                    * np.sin((np.pi / 180) * 0.5 * (lons - lon)) ** 2
                )
            )
        )  # distances from current site to all included sites

        if (
            np.sum(distances < min_dist) > 1
        ):  # if other tgs than current tg within 3 km:
            if length != np.max(
                lengths[distances < min_dist]
            ):  # if current record shorther than nearby records
