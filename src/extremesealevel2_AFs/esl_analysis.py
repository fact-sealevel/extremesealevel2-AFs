...
def ESL_stats_from_gtsm_dmax(
    gtsm_path, input_locations, preproc_settings, match_dist_limit, output_dir=None
):

    print(
        "Warning: Several preprocessing options applicable to GESLA data are bypassed for analyzing GTSM data, such as minimum length, MSL reference, and modeling events below the GPD threshold."
    )

    assert (preproc_settings["extremes_threshold"] > 0) & (
        preproc_settings["extremes_threshold"] < 100
    )

    if preproc_settings["resample_freq"] != "D_max":
        raise Exception(
            "Configured resample frequency not yet implemented. Assuming daily maxima Gtsm data."
        )
    ...
    threshold = gtsm.waterlevel.quantile(
        preproc_settings["extremes_threshold"] / 100, dim="time"
    )
    extremes = gtsm.waterlevel.where(gtsm.waterlevel >= threshold)
    extremes_declustered = extremes.where(
        (
            extremes
            == extremes.rolling(
                time=preproc_settings["declus_window"], center=True, min_periods=1
            ).max()
        )
        & (np.isfinite(extremes))
    )  # note this only works if resolution is daily!!

    if preproc_settings["store_esls"]:
        extremes_declustered.to_netcdf(
            os.path.join(output_dir, "GTSM_dmax_declustered_extremes.nc"), mode="w"
        )

    avg_extr_pyear = (
        365.25 * np.isfinite(extremes_declustered).sum(dim="time") / len(gtsm.time)
    )

    print("Fitting GPD to extremes.")
    gpd_params, nlogl, cov_mats = xr.apply_ufunc(
        fit_gpd_to_extremes_ds,
...
def ESL_stats_from_raw_GESLA(
    ...
    if gesla_version == "gesla2":
        (station_names, station_lats, station_lons, station_filenames) = (
            extract_GESLA2_locations(path_to_gesla, preproc_settings["min_yrs"])
        )  # extract_GESLA2_locations('/Volumes/Naamloos/PhD_Data/GESLA2/private_14032017_public_110292018/')
    elif gesla_version == "gesla3":
        (station_names, station_lats, station_lons, station_filenames) = (
            extract_GESLA3_locations(path_to_gesla, preproc_settings["min_yrs"])
        )
    else:
        raise Exception("Data source not recognized.")

    min_idx = [
        mindist(x, y, station_lats, station_lons, match_dist_limit)
        for x, y in zip(input_locations.lat.values, input_locations.lon.values)
    ]  # sites within maximum distance from input_locations points
    matched_filenames = []
    sites_with_esl = []
    for i in np.arange(
        len(input_locations.locations)
    ):  # loop over input_locations sites
        if min_idx[i] is not None:  # if nearby ESL data found, try to analyze that data
            matched_filenames.append(
                [station_filenames[x] for x in min_idx[i]]
            )  # esl stations within distance to sea-level projection site
            sites_with_esl.append(
                input_locations.locations[i].values
            )  # sea-level projections sites with nearby ESL data
        else:
            print(
                "Warning: No nearby ESL information found for {0}. Skipping site.".format(
                    input_locations.locations[i].values
                )
            )
    ...
    if not matched_filenames:
        raise Exception(
            "Zero matches found within {} km for provided lat/lon list".format(
                match_dist_limit
            )
        )
    ...
    if preproc_settings["store_esls"]:
        if not os.path.exists(os.path.join(output_dir, "ESLs")):
            os.mkdir(os.path.join(output_dir, "ESLs"))

    # Loop over the matched files
    print("Analyzing GESLA records.")
    for i in tqdm(
        np.arange(len(sites_with_esl))
    ):  # loop over sea-level projection sites with matched ESL data
        # This ID
        ...
        for esl_file in matched_filenames[i]:
            ...
            try:
                ...
                if gesla_version == "gesla2":
                    dfs = ingest_GESLA2_files(
                        path_to_gesla, preproc_settings, fns=[esl_file]
                    )

                elif gesla_version == "gesla3":
                    dfs = ingest_GESLA3_files(
                        path_to_gesla, preproc_settings, fns=[esl_file]
                    )
                ...
                extremes = pot_extremes_from_gesla_dfs(
                    dfs,
                    preproc_settings["extremes_threshold"],
                    preproc_settings["declus_method"],
                    preproc_settings["declus_window"],
                )
                ...
                esl_statistics[this_id]["matched_lon"] = dfs[esl_file].attrs[
                    "longitude"
                ]
                ...
                print(f"(esl_analysis 184) This file {esl_file} failed with {e}")
                continue

        # Let the user know we didn't find a file that passes the data constraints
        if not this_id_passed:
            print(
                "No locations within {0} km pass the data constraints for ID {1}.".format(
                    match_dist_limit, this_id
                )
            )
...
def pot_extremes_from_gesla_dfs(
    dfs, threshold_pct, declus_method=None, declus_window=None
):
    ...
    for k, df in dfs.items():
        ...
        if declus_method == "iterative_descending":
            ...
            extremes_sorted = extremes.sort_values(
                by=["sea_level"], ascending=False, kind="mergesort"
            )

            decl_extremes = extremes_sorted.iloc[
                [0]
            ]  # initialize using the first (highest) extreme

            for i in range(1, len(extremes_sorted)):  # for each subsequent extreme
                next_extreme = extremes_sorted.iloc[[i]]
                timediffs = (
                    decl_extremes.index - next_extreme.index[0]
                ...
                if all(
                    abs(timediffs / np.timedelta64(24 * 3600, "s")) >= declus_window
                ):
                    decl_extremes = pd.concat(
                        (decl_extremes, next_extreme)
                    )  # add next extreme
            ...
            extremes_dfs[k] = (
                ...
                .where(
                    ...
                    == extremes["sea_level"]
                    .rolling(
                        window=str(declus_window) + "D", center=True, min_periods=1
                    )
                    .max()
                )
                .dropna()
        ...
        elif declus_method is None:
            extremes_dfs[k] = extremes

        if "H" in df.attrs["resample_freq"]:
            rps = pd.DataFrame(
                data=[
                    (1 + len(df) / 365.25 / 24) / i
                    for i in range(1, len(extremes_dfs[k]) + 1)
                ],
                index=extremes_dfs[k]
                .sort_values(by=["sea_level"], ascending=False, kind="mergesort")
                .index,
                columns=["rp_empirical"],
            )  # determine return periods of declustered extremes empirically
            extremes_dfs[k] = extremes_dfs[k].join(rps.sort_index())

            extremes_dfs[k].attrs["avg_extr_pyear"] = (
                24 * 365.25 * len(extremes_dfs[k]) / len(df)
            )
            ...
            extremes_dfs[k].attrs["avg_extr_pyear"] = (
                365.25 * len(extremes_dfs[k]) / len(df)
            )

        # add metadata
        extremes_dfs[k].attrs["mhhw"] = np.nanmean(
            df.groupby(pd.Grouper(freq="2D")).max()
        )  # 2d because some places don't see high tide every day
...
def guess_initial_gpd_params(data):
    ...
    k0 = -0.5 * ((xbar**2) / s2 - 1)  # initial guesses
    sigma0 = 0.5 * xbar * ((xbar**2) / s2 + 1)
    xmax = max(data)

    if (
        k0 < 0 and xmax >= -sigma0 / k0
    ):  # if method of moments invalid (code based on MATLAB script gpfit.m), #assume exponential distribution
        k0 = 0
        sigma0 = xbar
...
def fit_gpd_to_extremes_ds(extremes_minus_loc):
    ...
    gp_params = genpareto.fit(
        extremes_minus_loc, loc=0, scale=sigma0, floc=0
    )  # fit gpd based on ESLs and initial guess, note that optimization vals differ slightly from using gpfit.m
    gp_nlogl, gp_cov = gplike(
        gp_params[0], gp_params[2], extremes_minus_loc
    )  # calculate covariance matrix of estimated parameters

    return (
        np.array([gp_params[-1], gp_params[0]]),
        gp_nlogl,
        gp_cov,
    )  # scale/shape params, nlogl MLE diagnostic, scale/shape cov
...
def fit_gpd_to_gesla_extremes(dfs):
    ...
    for k, df in dfs.items():
        ...
        gp_params = genpareto.fit(
            df["sea_level"].values - loc, loc=0, scale=sigma0, floc=0
        )  # fit gpd based on ESLs and initial guess, note that optimization vals differ slightly from using gpfit.m #TH edit 8-2-24: may have to do with not fixing floc=0 as in gpfit.m, changed this
        gp_nlogl, gp_cov = gplike(
            gp_params[0], gp_params[2], df["sea_level"].values - loc
        )  # calculate covariance matrix of estimated parameters
        ...
        d = {
            ...
            "shape": [gp_params[0]],
            "cov": [gp_cov],
            "avg_extr_pyear": df.attrs["avg_extr_pyear"],
            "mhhw": df.attrs["mhhw"],
            "key": [k],
        }
...
def multivariate_normal_gpd_samples_from_covmat(scale, shape, cov, n, seed=1234):
    '''generate scale and shape samples assuming a multivariate normal distribution with covariance matrix "cov"'''
    # if seed:
    #    np.random.seed(seed)
    rng = np.random.default_rng(seed)
    gp_samples = rng.multivariate_normal([shape, scale], cov, size=n)
    # gp_samples = np.random.multivariate_normal([shape,scale], cov,size=n)
    shape_samples = gp_samples[:, 0]
    scale_samples = gp_samples[:, 1]
    scale_samples[scale_samples < 0] = (
        0.00001  # no negative scales, assign very small scale instead (completely 0 leads to division problems)
    )
...
def gum_amax_Z_from_F(scale, loc, f):
    ...
    try:  # convert array input to float if using only 1 scale
        ...
        pass  # scale is already a Python scalar, no conversion needed
    except ValueError:
        pass  # scale is a multi-element array; keep as-is for vectorized operations
    f = np.where(f < 1, f, np.nan)  # equation supported to f=1?

    z = 0 * f  # initialize z
    z = (
        loc - np.log(-np.log(1 - f)) * scale
    )  # see Eq 3.4 in Coles; inverted from cdf formula exceedeance prob = 1-np.exp(-np.exp((location-z)/scale)
...
def gpd_Z_from_F(scale, shape, loc, avg_exceed, f):
    ...
    try:  # convert array input to float if using only 1 shape & scale
        ...
        pass  # scale/shape are already Python scalars, no conversion needed
    except ValueError:
        pass  # scale/shape are multi-element arrays; keep as-is for vectorized operations
    ...
    if np.isscalar(shape):  # if shape is scalar value
        if shape == 0:
            z = (
                -scale * np.log(f / avg_exceed)
            )  # rearranged from F = f(Z), as in e.g., Frederikse et al. (2020), Buchanan et al. (2016)
        else:
            z = (
                scale / shape * (np.power((f / avg_exceed), (shape / -1)) - 1)
            )  # rearranged from F = f(Z), as in e.g., Frederikse et al. (2020), Buchanan et al. (2016)
        ...
        if np.any(shape == 0):
            ...
            z[shape != 0] = (
                scale[shape != 0]
                / shape[shape != 0]
                * (np.power((f[shape != 0] / avg_exceed), (shape[shape != 0] / -1)) - 1)
            )
        else:
...
def gpd_Z_from_F_mhhw(scale, shape, loc, avg_exceed, f, mhhw):
    ...
    z[f > avg_exceed] = (
        0
        + np.log(f[f > avg_exceed] / avg_exceed)
        * (mhhw - loc)
        / np.log((365.25 / 2) / avg_exceed)
    )  # lower bound of gumbel is z=z0-loc=(mhhw-loc), i.e. (loc-mhhw) below loc if mhhw follows from long-term mean of 2-day maximum
...
def gpd_Z_from_F_Sweet22(scale, shape, loc, avg_exceed, f):
    ...
    z_0p5 = (
        gpd_Z_from_F(scale, shape, loc, avg_exceed, np.array([0.5])) - loc
    )  # heights corresponding to 0.5/yr frequency
    z_0p2 = (
        gpd_Z_from_F(scale, shape, loc, avg_exceed, np.array([0.2])) - loc
    )  # heights corresponding to 0.2/yr frequency

    iXtrp = (f > avg_exceed) & (
        f <= 10
    )  # indices of frequencies to fill with extrapolation

    if (avg_exceed > 0.5) & (avg_exceed < 10):
        if np.isscalar(shape):  # if shape is scalar value
            z[iXtrp] = z_0p5 + np.log(f[iXtrp] / 0.5) * (0 - z_0p5) / np.log(
                avg_exceed / 0.5
            )  # log-linear similar to Gumbel
        else:
            z[iXtrp] = z_0p5[iXtrp] + np.log(f[iXtrp] / 0.5) * (
                0 - z_0p5[iXtrp]
            ) / np.log(avg_exceed / 0.5)

    elif (avg_exceed > 0.2) & (avg_exceed < 10):
        if np.isscalar(shape):  # if shape is scalar value
            z[iXtrp] = z_0p2 + np.log(f[iXtrp] / 0.2) * (0 - z_0p2) / np.log(
                avg_exceed / 0.2
            )  # log-linear similar to Gumbel
        else:
            z[iXtrp] = z_0p2[iXtrp] + np.log(f[iXtrp] / 0.2) * (
                0 - z_0p2[iXtrp]
            ) / np.log(avg_exceed / 0.2)

    if avg_exceed <= 0.2:
        print(
            'Warning: "avg_extr_pyear" is equal to or less than 0.2/yr. Cannot extrapolate using the method of Sweet et al. (2022).'
        )
    # return z #output relative to location parameter
    return z + loc  # output relative to vertical datum
