...
def open_input_locations(path_to_input, num_samps):
    ...
    input_locations["locations"] = input_locations.locations.astype(
        "str"
    )  # to use them as keys for dictionary
    ...
    if ("lon" not in input_locations) or ("lat" not in input_locations):
        raise ValueError(
            "input_locations must contain 'lon' and 'lat' coordinate variables"
        )
    ...
    if (
        "sea_level_change" in input_locations
    ):  # determine if file also contains sea-level projections:
        if (
            "samples" in input_locations.sea_level_change.dims
        ):  # randomly select "num_mc" samples
            if num_samps > len(input_locations.samples):
                raise Exception(
                    "Insufficient SLR samples for desired number of Monte Carlo samples."
                )
            else:
                idx = np.sort(
                    np.random.default_rng(seed=5678).choice(
                        len(input_locations.samples), size=num_samps, replace=False
                    )
                )
                input_locations = input_locations.isel(samples=idx)
        else:
            raise Exception(
                "SLR projections do not contain samples required for uncertainty propagation."
            )

        if input_locations.sea_level_change.units != "m":
            if input_locations.sea_level_change.units == "mm":
                input_locations["sea_level_change"] = (
                    input_locations["sea_level_change"] / 1000
                )
                input_locations.sea_level_change.attrs["units"] = "m"
            elif input_locations.sea_level_change.units == "cm":
                input_locations["sea_level_change"] = (
                    input_locations["sea_level_change"] / 100
                )
                input_locations.sea_level_change.attrs["units"] = "m"
            else:
...
def esl_statistics_dict_to_ds(input_locations, esl_statistics):
    ...
    ds = xr.Dataset(
        data_vars=dict(
            loc=(
                ["locations"],
                [v["loc"].values[0] for k, v in esl_statistics.items()],
            ),
            scale=(
                ["locations"],
                [v["scale"].values[0] for k, v in esl_statistics.items()],
            ),
            shape=(
                ["locations"],
                [v["shape"].values[0] for k, v in esl_statistics.items()],
            ),
            cov=(
                ["locations", "i", "j"],
                [v["cov"].values[0] for k, v in esl_statistics.items()],
            ),
            avg_extr_pyear=(
                ["locations"],
                [v["avg_extr_pyear"].values[0] for k, v in esl_statistics.items()],
            ),
        ),
        coords=dict(
            lon=(
                ["locations"],
                input_locations.sel(locations=list(esl_statistics.keys())).lon.values,
            ),
            lat=(
                ["locations"],
                input_locations.sel(locations=list(esl_statistics.keys())).lat.values,
            ),
            locations=input_locations.sel(
                locations=list(esl_statistics.keys())
            ).locations.values,
            matched_lon=(
                ["locations"],
                [v["matched_lon"].values[0] for k, v in esl_statistics.items()],
            ),
            matched_lat=(
                ["locations"],
                [v["matched_lat"].values[0] for k, v in esl_statistics.items()],
            ),
            matched_id=(
                ["locations"],
                [v["matched_id"].values[0] for k, v in esl_statistics.items()],
            ),
        ),
    )
    ...
    ds["mhhw"] = (
        ["locations"],
        [v["mhhw"].values[0] for k, v in esl_statistics.items()],
    )  # mhhw is optional in esl_statistics
    # except:
    #    pass
    ...
    ds["vdatum"] = (
        ["locations"],
        [v["vdatum"].values[0] for k, v in esl_statistics.items()],
    )
    # except:
    #    pass
    return ds
...
def lazy_output_to_ds(
    ...
    if len(target_years) > 0:
        output_ds["z_fut"] = (
            ["locations", "qnt", "f", "target_year"],
            np.stack([k[1] for k in output]),
        )
        output_ds["AF"] = (
            ["locations", "qnt", "target_year"],
            np.stack([k[2] for k in output]),
        )
    ...
    if len(target_AFs) > 0:
        output_ds["AF_timing"] = (
            ["locations", "qnt", "target_AF"],
            np.stack([k[4] for k in output]),
        )
        output_ds = output_ds.assign_coords({"target_AF": target_AFs})

    if len(target_freqs) > 0:
        output_ds["f_timing"] = (
            ["locations", "qnt", "target_f"],
            np.stack([k[5] for k in output]),
        )
...
def find_flopros_protection_levels(qlons, qlats, flopros_dir, maxdist):
    ...
    polygons = geopd.read_file(
        os.path.join(
            flopros_dir, "Results_adaptation_objectives/Countries_States_simplified.shp"
        )
    )  # shape file with region polygons
    wb = load_workbook(
        filename=os.path.join(flopros_dir, "FLOPROS_geogunit_107.xlsx")
    )  # protection standards for each region
    ws = wb.active
    ...
    for qlat, qlon in tqdm(zip(qlats, qlons)):  # loop over query coordinates
        ...
        i_near = polygons.iloc[
            nearby
        ].index[
            np.argsort([p.distance(k) for k in polygons.iloc[nearby].geometry])[0:3]
        ]  # find the nearest 3 segments based on euclidean distances (default for shapely)

        # for these segments, refine the distance computation using a coordinate-specific appropriate reference system
        p_gdf = geopd.GeoDataFrame(
            geometry=[p], crs="epsg:4326"
        )  # put query point into cartesian geodataframe
        ...
        polygons_ = polygons.iloc[i_near].to_crs(
            epsg=e
        )  # convert coordinates of 3 nearest diva segments to epsg system

        km_dists = (
            np.array([p_gdf.distance(k).values[0] for k in polygons_.geometry]) / 1000
        )  # compute kilometric distances

        if np.min(
            km_dists > maxdist
        ):  # if nearest point on nearest segment is more than max dist km away
            nearest_segments.append(np.nan)
            # add a warning?
            continue

        nearest_segments.append(
            polygons_.index[np.argmin(km_dists)]
        )  # append index of nearest diva segment
    ...
    for k in nearest_segments:
        if np.isfinite(k):
            ...
            protection_levels.append(
                1 / plevel
            )  # add protection level to list in units of return frequency
        else:
...
def find_diva_protection_levels(qlons, qlats, diva_fn, maxdist):
    ...
    for qlat, qlon in tqdm(zip(qlats, qlons)):  # loop over query coordinates
        ...
        i_near = diva.iloc[
            ...
            np.argsort([p.distance(k) for k in diva.iloc[nearby].geometry])[0:5]
        ]  # find the nearest 5 segments based on euclidean distances (default for shapely)
        ...
        p_gdf.to_crs(epsg=e, inplace=True)  # convert point to epsg system
        diva_ = diva.iloc[i_near].to_crs(
            epsg=e
        )  # convert coordinates of 5 nearest diva segments to epsg system

        km_dists = (
            np.array([p_gdf.distance(k).values[0] for k in diva_.geometry]) / 1000
        )  # compute kilometric distances

        if np.min(
            km_dists > maxdist
        ):  # if nearest point on nearest segment is more than 15km away
            nearest_segments.append(np.nan)
            # add a warning?
            continue

        nearest_segments.append(
            diva_.index[np.argmin(km_dists)]
        )  # append index of nearest diva segment
    ...
    for k in nearest_segments:
        if np.isfinite(k):
            plevel = diva.protection_level_modelled.values[
                k
            ]  # in units of return period

            if (
                plevel == 0
            ):  # if no protection, assign protection of 1/2y (following Scussolini et al., 2016)
                plevel = 2
            protection_levels.append(
                1 / plevel
            )  # add protection level to list in units of return frequency
    ...
    for i in np.arange(len(protection_levels)):  # for each input location
        if min_idx[i] is not None:  # if nearby ESL information found
            try:
                ...
                # min_idx[i] is a scalar integer (single match), not subscriptable
                print(
                    f"Warning: min_idx[{i}] is not subscriptable, treating as scalar: {e}"
                )
                j = min_idx[i]
...
def get_refFreqs(refFreq_data, input_locations, esl_statistics, path_to_refFreqs=None):
    ...
    if refFreq_data == "diva":  # find contemporary DIVA flood protection levels
        refFreqs = find_diva_protection_levels(
            qlons, qlats, path_to_refFreqs, 10
        )  # set these paths in config
        ...
        refFreq_data
    ):  # use constant reference frequency for every location
        refFreqs = np.tile(refFreq_data, len(esl_statistics.locations))
    else:
        raise Exception(
            'Rquested reference frequency must be "diva", "flopros","custom" or a constant.'
        )

    return refFreqs


def open_gpd_parameters(
    input_data, data_path, input_locations, n_samples, match_dist_limit
):
    if input_data == "hermans2023":
        gpd_params = xr.open_dataset(data_path)  # open GPD parameters
        min_idx = [
            mindist(
                x, y, gpd_params.lat.values, gpd_params.lon.values, match_dist_limit
            )
            for x, y in zip(input_locations.lat.values, input_locations.lon.values)
        ]  # find ESL indices within radius of input locations if any
        ...
        if len(gpd_params.nboot) > n_samples:  # determine which samples to select
            ...
            isamps = np.hstack(
                (
                    gpd_params.nboot.values,
                    np.random.randint(
                        0, len(gpd_params.nboot), n_samples - len(gpd_params.nboot)
                    ),
                )
            )
        else:
        ...
        for i in np.arange(len(input_locations.locations)):  # for each input location
            if min_idx[i] is not None:  # if nearby ESL information found
                if not np.isscalar(
                    min_idx[i]
                ):  # if multiple, pick the first (we don't have information about series length here)
                    iEsl.append(min_idx[i][0])
                else:
                    iEsl.append(min_idx[i])  # if only one, use that
                iLocations.append(
                    i
                )  # nearby ESL information for this site, so store site
        ...
        esl_statistics = gpd_params.isel(site=iEsl).isel(
            nboot=isamps
        )  # selected sites & samples
        esl_statistics = esl_statistics.rename(
            {
                "avg_exceed": "avg_extr_pyear",
                "nboot": "samples",
                "location": "loc",
                "site": "locations",
            }
        )  # rename some variables
        esl_statistics = esl_statistics[
            [
                "loc",
                "avg_extr_pyear",
                "scale",
        ...
        esl_statistics["locations"] = input_locations.isel(
            locations=iLocations
        ).locations
        esl_statistics = esl_statistics.assign_coords(
            {
        ...
        esl_statistics = xr.Dataset(
            data_vars=dict(
                loc=(["locations"], gpd_params.iloc[iEsl].GPDthresh.values),
                scale=(["locations"], gpd_params.iloc[iEsl].GPDscale.values),
                shape=(["locations"], gpd_params.iloc[iEsl].GPDshape.values),
                avg_extr_pyear=(
                    ["locations"],
                    infer_avg_extr_pyear(
                        gpd_params.iloc[iEsl].GPDthresh.values,
        ...
        gpd_params = gpd_params.rename(
            {
                "thresholdParamGPD": "loc",
                "scaleParamGPD": "scale",
                "shapeParamGPD": "shape",
                "npt": "locations",
                "pt": "locations",
        ...
        esl_statistics = esl_statistics.assign_coords(
            {
                "matched_id": ("locations", matched_ids),
                "matched_lon": ("locations", esl_statistics.lon.values),
                "matched_lat": ("locations", esl_statistics.lat.values),
            }
        )
        esl_statistics["avg_extr_pyear"] = infer_avg_extr_pyear(
            esl_statistics["loc"],
            esl_statistics.scale,
            esl_statistics.shape,
            esl_statistics.returnlevelGPD.isel(return_period=12),
            1 / 100,
        )
...
def get_coast_rp_return_curves(input_dir, input_locations, f, match_dist_limit):
    ...
    min_idx = [
        mindist(
            x,
            y,
            coast_rp_coords["lat"].values,
            coast_rp_coords["lon"].values,
            match_dist_limit,
        )
        for x, y in zip(input_locations.lat.values, input_locations.lon.values)
    ]
    ...
    for i in np.arange(len(input_locations.locations)):
        if min_idx[i] is not None:
            if not np.isscalar(
                min_idx[i]
            ):  # if multiple input_locations within radius, pick the first (we don't have information about series length here)
                min_idx[i] = min_idx[i][0]
            this_id = str(int(coast_rp_coords.iloc[min_idx[i]].name)).rjust(5, "0")
            # try:
            rc = pd.read_pickle(
                os.path.join(input_dir, "rp_full_empirical_station_" + this_id + ".pkl")
            )
            # except:
    ...
    esl_statistics = xr.Dataset(
        data_vars=dict(z_hist=(["locations", "f"], np.stack(zs))),
        coords=dict(
            locations=input_locations.isel(locations=iLocations).locations, f=f
        ),
    )
    esl_statistics = esl_statistics.assign_coords(
        {
            "matched_lon": ("locations", coast_rp_coords.iloc[iMatched].lon.values),
...
def open_gtsm_waterlevels(gtsm_dir, input_locations, match_dist_limit):
    gtsm = xr.open_mfdataset(
        os.path.join(gtsm_dir, "*.nc")
    )  # assumes daily maxima in similar file structure as original CDS download
    gtsm_lats = gtsm.station_y_coordinate.values
    gtsm_lons = gtsm.station_x_coordinate.values

    min_idx = [
        mindist(x, y, gtsm_lats, gtsm_lons, match_dist_limit)
        for x, y in zip(input_locations.lat.values, input_locations.lon.values)
    ]
    for k in np.arange(len(input_locations.locations)):
        if min_idx[k] is not None:
            ...
            print(
                "Warning: No nearby ESL information found for {0}. Skipping site.".format(
                    input_locations.locations[k].values
                )
            )
    matched_ids = gtsm.stations.values[
        np.array(min_idx)[np.isfinite(min_idx)].astype("int")
    ]
    gtsm = gtsm.isel(stations=np.array(min_idx)[np.isfinite(min_idx)].astype("int"))
    gtsm = gtsm.rename(
        {
            "stations": "locations",
            "station_x_coordinate": "matched_lon",
            "station_y_coordinate": "matched_lat",
        }
    )
    gtsm = gtsm.assign_coords(
        {
            "matched_id": ("locations", matched_ids),
            "lon": ("locations", input_locations.lon[np.isfinite(min_idx)].values),
            "lat": ("locations", input_locations.lat[np.isfinite(min_idx)].values),
        }
    )
    ...
    gtsm = gtsm.drop_isel(
        locations=np.where(np.isnan(gtsm.waterlevel).any(dim="time"))[0]
    )  # remove stations containing nans in their timeseries
    gtsm = gtsm.where(
        (gtsm.waterlevel.var(dim="time") > 1e-5)
    ).dropna(
        dim="locations"
    )  # remove weird stations with very low variance (erroneous, sea ice, internal seas?)
