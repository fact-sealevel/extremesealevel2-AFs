...
def kmdist(qlat, qlon, lats, lons):
    ...
    distances = 2 * np.arcsin(
        np.sqrt(
            ...
            + np.cos((np.pi / 180) * qlat)
            * np.cos((np.pi / 180) * lats)
            * np.sin((np.pi / 180) * 0.5 * (lons - qlon)) ** 2
        )
    )
...
def download_ar6_full_sample_projections_at_tgs(wf, ssp, out_dir):  # ~1.2Gb!
    ...
    return ds.to_zarr(
        os.path.join(out_dir, "full_sample_total_" + wf + "_" + ssp + ".zarr"), mode="w"
    )
...
def add_ar6_full_sample_projections_to_locations(
    input_locations, slr_fn, nsamps, period
):
    ...
    mask = (
        np.isnan(ds.isel(years=0, samples=0).sea_level_change)
        .stack(locations=["lon", "lat"])
        .values
    )  # get land mask
    min_idx = [
        np.argmin(kmdist(qlat, qlon, slr_lats, slr_lons) + 999 * mask)
        for qlat, qlon in zip(input_locations.lat.values, input_locations.lon.values)
    ]  # get nearest projections to input_locations, don't use land

    ds_at_input_locations = ds_stacked.isel(locations=min_idx).drop(
        "lat"
    )  # drop multiindex
    ds_at_input_locations["locations"] = input_locations[
        "locations"
    ].values  # assign locations from input_locations file

    input_locations["sea_level_change"] = ds_at_input_locations[
        "sea_level_change"
    ]  # add SLC variable to input_locations file
    input_locations = input_locations.sel(
        years=slice(str(period[0]), str(period[1]))
    ).isel(
        samples=np.arange(nsamps)
    )  # select requested number of samples (samples are randomly ordered in the input) and period
