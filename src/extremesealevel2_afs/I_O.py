...
def open_input_locations(path_to_input,num_samps):
    ...
    if ('lon' not in input_locations) or ('lat' not in input_locations):
        raise Exception as e ('Input_locations input must contain lon/lat coordinates...', e)
    ...
    input_locations.to_netcdf(
        os.path.join(
            os.path.dirname(__file__),'aaa_input_locations.nc'),
        mode='w',
        encoding=encoding) #store output ds
...
def open_gtsm_waterlevels(gtsm_dir, input_locations, match_dist_limit):
    ...
    gtsm = gtsm.assign_coords({'matched_id':
                               ('locations',matched_ids),'lon':
                               ('locations', input_locations.lon[np.isfinite(min_idx)].values),
