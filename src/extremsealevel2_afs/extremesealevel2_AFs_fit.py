...
def get_ESL_statistics(
    ...
    if esl_data in [
        "gesla2",
        "gesla3",
    ]:  # options were gesla2 adn gesla3 #from raw GESLA data
        esl_statistics = ESL_stats_from_raw_GESLA(
            ...
            match_dist_limit=match_dist_limit,
        )  # dictionary output
        ...
        esl_statistics = ESL_stats_from_gtsm_dmax(
            path_to_data, input_locations, preproc_settings, match_dist_limit
        )

    elif esl_data.lower() in [
        "hermans2023",
        "kirezci2020",
        "vousdoukas2018",
        "gtsm_dmax_gpd",
    ]:  # from pre-made statistics
        esl_statistics = open_gpd_parameters(
            esl_data, path_to_data, input_locations, n_samples, match_dist_limit
        )

    elif esl_data.lower() == "coast-rp":  # from COAST-RP return curves
        esl_statistics = get_coast_rp_return_curves(
            path_to_data, input_locations, f, match_dist_limit
        )
    else:
